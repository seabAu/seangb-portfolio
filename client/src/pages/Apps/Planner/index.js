...
function Planner(props) {
	...
	useEffect(() => {
		...
		if (utils.val.isValidArray(appsData.tasks)) {
			...
			setFormDataModel(utils.data.initializeModel(appsData.tasks[0]));
		}
	}, [appsData]);

	useEffect(() => {
		// On initial mount, build data model.
		// setFormDataModel(utils.data.initializeModel(taskRenderData));
	}, [taskRenderData]);
	...
	return (
		<div
			...
			style={componentStyles}>
			{show && (utils.val.isValidArray(children, true) || utils.val.valIsValid(children)) && children}
