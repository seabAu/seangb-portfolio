...
function Pane(props) {
    ...
    return (
        showParent && (
            <div
                className={`section-pane section-${layout === "col" ? "col" : layout === "row" ? "row" : layout === "mosaic" ? "mosaic" : "flex" } section-pane-${
                    behavior === "spread" ? "spread" : behavior === "shrink" ? "shrink" : "wrap"
                }`}
                style={
                    {}
                    // componentStyles
...
function Separator(props) {
    ...
    return (
        <div
            ...
            id={`section-bar-${getStyle(type)}-${Math.random() * 100000}`}
            key={`section-bar-${getStyle(type)}-${Math.random() * 100000}`}>
            <hr
...
function Content(props) {
    ...
    return (
		showParent && (
			<div
				className={`section-content${classes ? classes : ""} section-${
					layout === "col" ? "col" : layout === "row" ? "row" : layout === "mosaic" ? "mosaic" : "flex"
...
function List(props) {
    ...
    const data2Text = (data, keys = []) => {
        ...
        if ( utils.val.isObject( data ) && utils.val.isValidArray( Object.keys( data ), true ) && utils.val.isValidArray( keys, true ) )
        {
            keys.forEach( ( key, index ) =>
            {
                ...
                text.push(
					<li
						className={`list-item section-list-item`}
    ...
    const buildContent = (content, keys) => {
        return (
            <ul
                className={`list section-list ${classes ? classes : ""}`}
                style={
                    {}
                    // componentStyles
                }>
    ...
    const buildContent = (input, displayType) => {
        ...
        if (utils.val.isObject(input)) {
            ...
            displayContent.push(
                ...
                ////    alt={`${input.url}`}></img>,
                <a
                    key={`section-image-${imageId}-${Math.random() * 100}`}
                    id={`section-image-${imageId}-${Math.random() * 100}`}
                    href={`${linkUrl}`}
            ...
            <div
                ...
                <div
                    ...
                    <button
                        ...
                        key={`collection-pagination-button-prev-${Math.random() * 100}`}
                        id={`collection-pagination-button-prev-${Math.random() * 100}`}>
                        <FaAngleLeft />
                    </button>
                    <button
                        ...
                        key={`collection-pagination-button-next-${Math.random() * 100}`}
                        id={`collection-pagination-button-next-${Math.random() * 100}`}>
                        <FaAngleRight />
                    </button>
                    {indicators}
                </div>
