...
function Card(props) {
    ...
    return (
        <div
            ...
            {children && (utils.val.isValidArray(children, true) || utils.val.valIsValid(children)) && children}
        </div>
    );
}
...
function Grid(props) {
    ...
    const buildPageButton = (label, icon, id, onClick, classes) => {
        if (!id) id = `pagination-button-${Math.floor(Math.random() * 100000)}`;
        return (
            <li
                ...
                <Button
                    icon={{ icon }}
                    label={{ label }}
                    className={`pagination-button ${classes}`}
                    onClick={onClick}>
                </Button>
            </li>
        );
    ...
    const getPageButtons2 = (dataLen, page, numPerPage, parentID) => {
		...
		buttons.push(
			buildPageButton(
				"",
				<FaAngleDoubleLeft />,
				`cards-${parentID}-pagination-container-back`,
				() => {
			...
			if (Math.abs(i - page) < 3 || i === 0 || i === numButtons - 1) {
				buttons.push(
					buildPageButton(
						`${i}`,
						`${i}`,
						`cards-${parentID}-pagination-container-${i}`,
						(event) => {
							changePage(
								dataLen,
						...
						`pagination-button ${i === page ? "current-page-button" : ""}`,
					),
				);
			}
		}
		buttons.push(
			buildPageButton(
				"",
				<FaAngleDoubleRight />,
				`cards-${parentID}-pagination-container-next`,
				() => {
					changePage(dataLen, +page + 1, numPerPage);
				},
				`pagination-button`,
    ...
    const buildContainerPagination = ( input, index, length) =>
    {
        ...
        //     return (
        //         <div
        //             className={`cards-container-pagination`}
        //             style={{}}>
        //             <p className={`cards-container-pagination-info`}>
        //                 {`Viewing ${index * length} to ${index * length + length - 1 > input.length ? input.length : index * length + length - 1} of ${
        //                     input.length
        //                 } entries found.`}
        //             </p>
        //         </div>
    ...
    return (
        <div
            className={`cards-container cards-${gridLayout === "grid" ? gridLayout : gridLayout === "flex" ? gridLayout : "list"}`}
            style={
                {}
                // gridStyles
            }>
            {utils.val.isValidArray(gridItems, true) && buildCollection(getPageEntries(gridItems, pageIndex, pageLength, []))}
        </div>
    );
}
