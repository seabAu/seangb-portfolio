			...
			return input.forEach((field, index) => {
				...
				if (utils.ao.has(field, "inputProps")) {
					...
					if (utils.ao.has(inputProps, "defaultValue")) {
						...
						value = inputProps.defaultValue;
						dataType = utils.data.getType(value); // typeof value;
					}
				}
				if (debug)
					console.log(
...
function Data(props) {
					...
					<div className={`data-input-group input-group-inline`}>
						<Input.Field
							...
							<Input.Control
								...
								inputProps={{
									defaultValue: currentInput ? currentInput : ``,
				...
				if (utils.val.isObject(inputModel)) {
					if (Object.keys(inputModel).length > 0) {
						...
						Object.keys(inputModel).forEach((key, index) => {
							...
							let elementdatatype = utils.data.getType(value);
							let elementfieldtype = utils.data.getFieldType(value);
							if (debug)
								console.log(
															...
															return (
																<div className={`data-input-field-inline-controls`}>
																	<Input.Control
																		...
																		onKeyPress={(e) => {
																			let keynum = e.which;
																			if (debug) console.log("Key pressed on input: e = ", e, " :: keynum = ", keynum, " :: e.key = ", e.key);
																		...
																		onChange={(e) => {
																			...
																			if (debug) console.log("temp[key] = ", temp[key], " :: ");
										...
										<div className={`data-input-group input-group-inline`}>
											<Input.Field
												...
												<Input.Control
													...
													onKeyDown={(e) => {
														let keynum = e.which;
														if (debug) console.log("Key pressed on input: e = ", e, " :: keynum = ", keynum);
													}}
													onChange={(e) => {
														if (debug)
															console.log(
	...
	const buildComponents = (input, dataType = "array") => {
									...
									<div className="data-input-cell-buttons">
										...
										<Button
											...
											onClick={(e) => {
												...
												if (debug) console.log("createCell :: index = ", index, " :: inputData = ", inputData, " :: inputData[index] = ", inputData[index]);
		...
		return (
			<div className={`data-input-container`}>
				...
				<div className={`data-input-field-container`}>
					{
						// currentInputMode !== "none" && inputDataModel && (
						// 	<Droplist
					...
					{
						// currentInputMode !== "none" && currentInputData && (
						// 	<Droplist
						// 		label={`${`Current Input`}`}
	...
	const handleDataAdd = () => {
		...
		if (
			...
			// let temp = [...currentInputData];
			let temp = currentInputData;
			if (debug) console.log("[...inputData, temp] = ", [...inputData, temp]);
			// setCurrentInputData(inputDataModel);
			setInputData([...inputData, temp]);
			setData(datakey, [
				// setInputData([
