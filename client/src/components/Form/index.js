    ...
    const constructForm = (input) => {
        ...
        if (utils.val.isObject(input)) {
            ...
            Object.keys(input).forEach((key, index) => {
                ...
                let fieldType = utils.data.getFieldType(value);
                let dataType = utils.data.getType(value);
                ...
                if (typeof value === "object" && Array.isArray(value)) {
					...
					// if ( debug ) console.log("FORM.JS :: constructForm :: input = ", input, " :: ARRAY :: value = ", value, " :: dataType = ", dataType, " :: fieldType = ", fieldType, "\n Utility test :: ", utils.data.getFieldType(value), utils.data.getType(value));
					...
					<Input
						...
						onChange={(e) => {
							...
							let updatedData = utils.ao.deepFindSet(
								...
								utils.data.formatInputValue(e, fieldType),
							);
							onChange(updatedData);
							setFormData(updatedData);
						}}
...
function Schema (props) {
			...
			Object.keys(schema).forEach((key, index) => {
				...
				if (fieldSchema) {
					...
					if (dataType) {
						...
						field.fieldType = utils.data.dataType2fieldType(dataType);
						...
						if (initializeRandom) {
							if (utils.val.isValidArray(enums, true)) {
								...
								initialValue = utils.data.generateRandom(dataType, 10, 10);
							}
							...
							else initialValue = utils.data.typeToInitialDefault(dataType, defaultValue);
						} else {
							// Try to glean an initial value from the dataType itself.
							initialValue = utils.data.typeToInitialDefault(dataType, defaultValue);
						}
	...
	const buildSchemaField = (fieldData) => {
		...
		if (value === undefined || value === null) value = utils.data.typeToInitialDefault(dataType);
		if (debug)
			console.log(
